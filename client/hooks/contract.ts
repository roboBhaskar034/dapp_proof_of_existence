...
export const CONTRACT_ADDRESS =
  "CBVQ2M5VHF24YF2M22HEKH55X5KGJV5KM3TJHUSDV3EUBUVEZBFSMGGN";
...
// ============================================================
// Public Poll Dapp — Contract Methods
// ============================================================

/**
 * Create a new poll. Anyone can call this.
...
export async function createPoll(
  ...
  return callContract(
    "create_poll",
    [
      toScValString(question),
      nativeToScVal(options, { type: "vec<string>" }),
    ],
    caller,
    true
  );
...
/**
 * Cast a vote on a poll. Requires wallet connection.
...
/**
 * Get a single poll's details (read-only).
 * Calls: get_poll(poll_id: u32) -> Poll
 */
export async function getPoll(pollId: number, caller?: string) {
  return readContract("get_poll", [toScValU32(pollId)], caller);
}

/**
 * Get all polls (read-only).
 * Calls: get_all_polls() -> Vec<Poll>
 */
export async function getAllPolls(caller?: string) {
  return readContract("get_all_polls", [], caller);
}
